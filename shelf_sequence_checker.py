...
#   ONE condition governs the ENTIRE store (cooler) — shelves within the
#   same store are never mixed (e.g. shelf 2 passing via Condition 1 while
#   shelf 3 in that same store passes via Condition 2). The rule:
#       1. Evaluate every shelf N > 1 in the store under Condition 1 alone.
#       2. If ALL shelves in the store pass Condition 1, that's the store's
#          result — Condition 2 is never evaluated at all for this store.
#       3. If even ONE shelf in the store fails Condition 1, discard those
#          results entirely and re-evaluate EVERY shelf in the store under
#          Condition 2 instead — that becomes the store's result, regardless
#          of which individual shelves would or wouldn't have passed
#          Condition 1.
...
#   - maxshelfnumber     : total shelf count for this image's cooler (same
#                           value for every image in the group)
#   - shelfposition      : this specific image's rank within its cooler
#                           (1..N) — this is the number actually used to
#                           pick/apply the rule
#   - skusconsidered     : count of resolved (non-"Other") SKUs detected on
#                           this shelf, regardless of whether they had bbox
#                           coordinates
#   - compliance_flag    : 'Y' if shelf 1, or if the condition governing
#                           this shelf's STORE (Condition 1 if every shelf in
#                           the store passed it, otherwise Condition 2 for
#                           every shelf in the store) passed for this shelf;
#                           'N' otherwise
#   - remarks            : empty if 'Y'; otherwise a short note on which
#                           condition governed the store (the actual detail
#                           is in condition1_remarks/condition2_remarks)
#   - condition1_remarks : Condition 1's own result for this shelf, always
#                           computed regardless of which condition actually
#                           governs the store's compliance_flag — empty if
#                           Condition 1 passed for this shelf
#   - condition2_remarks : Condition 2's own result for this shelf, always
#                           computed regardless of which condition actually
#                           governs the store's compliance_flag — empty if
#                           Condition 2 passed for this shelf
...
# def _evaluate_condition1_all(shelf_items_by_rank: list) -> list:
#     """Evaluate every shelf N>1 under Condition 1 alone (top-to-bottom size
#     growth vs. the immediately previous shelf). Shelf 1 is always ('Y', '').
#     Returns (flag, remark) per shelf, same order as shelf_items_by_rank."""
...
#         if not prev_items:
#             results[i] = ('N', "condition1: previous shelf has no valid SKU detections")
#             continue
#         if not cur_items:
#             results[i] = ('N', "condition1: current shelf has no valid SKU detections")
#             continue
...
# def _evaluate_condition2_all(shelf_items_by_rank: list) -> list:
#     """Evaluate every shelf N>1 under Condition 2 alone (left-to-right size
#     growth on that shelf, independent of any other shelf). Shelf 1 is
#     always ('Y', ''). Returns (flag, remark) per shelf, same order as
#     shelf_items_by_rank."""
#     n = len(shelf_items_by_rank)
#     results = [None] * n
#     results[0] = ('Y', '')
...
#     for i in range(1, n):
#         cond2_pass, cond2_remark = _condition2_left_to_right(shelf_items_by_rank[i])
#         results[i] = ('Y', '') if cond2_pass else ('N', cond2_remark)
#     return results
...
#     Returns a list of (compliance_flag, remarks, condition1_remark,
#     condition2_remark) tuples, same length/order as shelf_items_by_rank —
#     i.e. result[i] is the verdict for shelf i+1.

#     ONE condition governs the ENTIRE store (cooler)'s compliance_flag —
#     shelves are never mixed, e.g. shelf 2 passing via Condition 1 while
#     shelf 3 passes via Condition 2 for the same store. The rule:
#         1. Evaluate every shelf under Condition 1 (top-to-bottom size
#            growth vs. the immediately previous shelf) alone.
#         2. If ALL shelves pass Condition 1, that's the store's result —
#            Condition 1 governs compliance_flag for every shelf.
#         3. If even ONE shelf fails Condition 1, Condition 2 (left-to-right
#            on that shelf alone) governs compliance_flag for EVERY shelf in
#            the store instead, regardless of which individual shelves
#            would or wouldn't have passed Condition 1.

#     Both conditions are ALWAYS evaluated for every shelf regardless of
#     which one governs — condition1_remark and condition2_remark carry
#     each condition's own result independently ('' if that condition
#     passed), so both are visible even for the condition that didn't end up
#     deciding the shelf's flag. `remarks` is just a short note on which
#     condition governed the store, for a quick read without needing both
#     detail columns.

#     Shelf 1 (top-most) is always ('Y', ...) under either condition, since
#     there's no shelf above it to compare against.
#     """
#     n = len(shelf_items_by_rank)
#     if n == 0:
#         return []

#     cond1_results = _evaluate_condition1_all(shelf_items_by_rank)
#     cond2_results = _evaluate_condition2_all(shelf_items_by_rank)

#     store_uses_cond1 = all(flag == 'Y' for flag, _remark in cond1_results)
#     governing_condition = 1 if store_uses_cond1 else 2

#     results = []
#     for i in range(n):
#         cond1_flag, cond1_remark = cond1_results[i]
#         cond2_flag, cond2_remark = cond2_results[i]
#         flag = cond1_flag if store_uses_cond1 else cond2_flag

#         if flag == 'Y':
#             remarks = ''
#         else:
#             remarks = (
#                 f"store evaluated under condition{governing_condition} "
#                 f"(see condition{governing_condition}_remarks)"
#             )
#         results.append((flag, remarks, cond1_remark, cond2_remark))
...
#             condition1_remarks varchar     NULL,
#             condition2_remarks varchar     NULL,
#             checked_at         timestamp   NOT NULL DEFAULT now(),
#             CONSTRAINT shelfsequencecompliance_pkey PRIMARY KEY (iterationid, iterationtranid)
#         )
#     """)
#     # For installs created before `shelfposition`/condition*_remarks existed.
#     cur.execute("""
...
#     cur.execute("""
#         ALTER TABLE orgi.shelfsequencecompliance
#         ADD COLUMN IF NOT EXISTS condition1_remarks varchar
#     """)
#     cur.execute("""
#         ALTER TABLE orgi.shelfsequencecompliance
#         ADD COLUMN IF NOT EXISTS condition2_remarks varchar
#     """)
...
#              compliance_flag, remarks, condition1_remarks, condition2_remarks,
#              checked_at)
...
#             condition1_remarks = EXCLUDED.condition1_remarks,
#             condition2_remarks = EXCLUDED.condition2_remarks,
#             checked_at         = now()
#     """
...
#             for rank, ((itranid, g), (flag, remarks, cond1_remark, cond2_remark)) in enumerate(zip(images, verdicts), start=1):
...
#                     cond1_remark[:500] if cond1_remark else None,
#                     cond2_remark[:500] if cond2_remark else None,
#                 ))
